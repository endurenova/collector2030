document.addEventListener('DOMContentLoaded', () => {
    ...
    artistVideoList.insertAdjacentHTML(
      ...
      <li>
        <div class="artist-video-thumbnail" data-video-idx="${e.idx}">
          <img src="/layout/basic/img/${e.thumbnail}" alt="${e.artist} thumbnail" />
          <span class="play_btn material-symbols-outlined"> play_circle </span>
        </div>
        <div class="artist-video-description">
          <img src="/layout/basic/img/${e.artistImg}" alt="${e.artist} image" />
  ...
  Array.from(videoArrow).forEach(e => {
    e.addEventListener('click', event => {
      ...
      const videoItemWidth = Number(getComputedStyle(videoThumbnailList[0]).getPropertyValue('width').split('px')[0]);
